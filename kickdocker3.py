...
DOCKER_IMAGE = "multitor3:latest"
CONTAINER_PREFIX = "multitor3_"
...
def calculate_token_settings(num_containers):
    ...
    if num_containers <= 30:
        TOKEN_POOL_SIZE = BASE_TOKEN_POOL_SIZE
        INITIAL_POOL_WAIT = BASE_INITIAL_POOL_WAIT
        TOKEN_PRODUCERS = BASE_TOKEN_PRODUCERS
    else:
        extra = num_containers - 30
        TOKEN_POOL_SIZE = BASE_TOKEN_POOL_SIZE + (extra * 30)
        INITIAL_POOL_WAIT = BASE_INITIAL_POOL_WAIT + (extra * 10)
        TOKEN_PRODUCERS = BASE_TOKEN_PRODUCERS + (extra * 3)
...
def build_image():
    print("[*] Building multitor3 image...")
    success, _ = run_cmd("docker build -t multitor3:latest -f Dockerfile.multitor3 .")
    return success
...
if __name__ == "__main__":
    try:
        os.system('cls' if os.name == 'nt' else 'clear')
        print("=== Kick Viewer Bot (Multi-Tor Docker 3-Port) ===\n")
        ...
        num_containers = int(input("Number of containers (e.g. 30): ").strip() or "30")
        ...
        print("[*] Waiting 30s for Tor instances to bootstrap...")
        time.sleep(30)
